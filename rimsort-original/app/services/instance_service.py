...
class InstanceService:
    ...
    def __init__(
        ...
        self._loading_manager: Any = None
    ...
    def _fire_loading(
        ...
        text: str,
        gif_name: str = "rimsort.gif",
    ) -> None:
        """Fire-and-forget: show modal overlay, run func in a thread, close when done."""
        gif_path = str(AppInfo().theme_data_folder / "default-icons" / gif_name)
        self.loading_manager.fire_and_forget(
            target=partial(func), text=text, gif_path=gif_path
        )

    # --- Instance lifecycle operations ---

    @asyncSlot(str)
    async def backup_existing_instance(self, instance_name: str) -> None:
        ...
        if output_path:
            ...
            with self.loading_manager.loading_state(
                QCoreApplication.translate(
                    "InstanceService",
                ...
                try:
                    await loop.run_in_executor(
                        None,
                        partial(instance_controller.compress_to_archive, output_path),
                    )
                except Exception as e:
                    show_fatal_error(
                        title=QCoreApplication.translate(
    ...
    @asyncSlot()
    async def restore_instance_from_archive(self) -> None:
        ...
        with self.loading_manager.loading_state(
            ...
            await loop.run_in_executor(
                None,
                partial(instance_controller.extract_from_archive, input_path),
            )
    ...
    @asyncSlot(str)
    async def clone_existing_instance(self, existing_instance_name: str) -> None:
        ...
        if (
            ...
            if answer.exec_is_positive():
                ...
                gif_path = str(AppInfo().theme_data_folder / "default-icons" / "rimworld.gif")
                with self.loading_manager.loading_state(
                    f"Cloning instance [{existing_instance_name}] to [{new_instance_name}]...",
                    gif_path,
                ):
                    loop = asyncio.get_running_loop()
