...
class AcfLogReader(BaseModsPanel):
    ...
    def _populate_from_metadata(self) -> None:
        """Base class override — handled asynchronously via _do_refresh."""
        pass
    ...
    async def _populate_from_metadata_async(self) -> None:
        """
        Asynchronously populate the ACF table from MetadataController's ACF and metadata.

        Data preparation runs in a thread pool; row insertion on the main thread
        is batched in small chunks with event-loop yields to keep the UI responsive.
        """
        await asyncio.sleep(0)
        logger.warning("ACF: _populate_from_metadata_async entered")
        ...
        try:
            ...
            logger.warning(
                f"ACF Log Reader: Populating with {len(acf_entries)} entries"
            )
            ...
            logger.warning(f"ACF Log Reader: PFID map built in {map_elapsed:.3f}s")

            # Build raw row data (pure data munging, no Qt/DB — runs on main thread,
            # but chunked insertion below yields to avoid blocking)
            raw_rows = self._prepare_acf_rows_data(acf_entries, pfid_to_mod)

            # Disable sorting + updates for bulk insertion
            self.editor_table_view.setSortingEnabled(False)
            self.editor_table_view.setUpdatesEnabled(False)

            # Chunked insertion on main thread — yields every BATCH_SIZE rows
            BATCH = 20
            start_row = self.editor_model.rowCount()
            insert_start = time.time()

            for batch_idx in range(0, len(raw_rows), BATCH):
                batch = raw_rows[batch_idx : batch_idx + BATCH]
                for row_data in batch:
                    ...
                    checkbox_item = QStandardItem("")
                    row_items = [checkbox_item] + base_items
                    self.editor_model.appendRow(row_items)

                # Yield to event loop so repaint / input events get processed
                # QApplication.processEvents()
                await asyncio.sleep(0)

            # Add interactive widgets (checkboxes, path links, workshop buttons) — chunked
            for batch_idx in range(0, len(raw_rows), BATCH):
                batch_end = min(batch_idx + BATCH, len(raw_rows))
                for i in range(batch_idx, batch_end):
                    row_idx = start_row + i
                    _, _, _, _, _, _, _, _, path, workshop_url, uuid = raw_rows[i]

                    checkbox = QCheckBox()
                    checkbox.setObjectName("selectCheckbox")
                    checkbox_idx = self.editor_model.item(row_idx, 0).index()
                    self.editor_table_view.setIndexWidget(checkbox_idx, checkbox)

                    if path and path.strip():
                        path_link = self._create_path_link(path, "pathLink")
                        path_idx = self.editor_model.item(
                            row_idx, ColumnIndex.PATH.value
                        ).index()
                        self.editor_table_view.setIndexWidget(path_idx, path_link)

                    if workshop_url:
                        workshop_button = self._create_workshop_button(
                            workshop_url, "workshopButton"
                        )
                        workshop_idx = self.editor_model.item(
                            row_idx, ColumnIndex.WORKSHOP_PAGE.value
                        ).index()
                        self.editor_table_view.setIndexWidget(
                            workshop_idx, workshop_button
                        )
            ...
            logger.warning(
                f"ACF Log Reader: Added {self.editor_model.rowCount()} rows in {insert_elapsed:.3f}s"
            )

            self.editor_table_view.setUpdatesEnabled(True)

            total_elapsed = time.time() - map_start
            logger.warning(
                f"ACF Log Reader: Population completed in {total_elapsed:.3f}s"
            )
            ...
            logger.warning(
                f"ACF: _populate_from_metadata_async cancelled after {time.time() - start_time:.3f}s"
            )
            self.editor_table_view.setUpdatesEnabled(True)
            self.editor_table_view.setSortingEnabled(True)
            raise
    ...
    @staticmethod
    def _prepare_acf_rows_data(
        acf_entries: list[tuple[str, str, int | None]],
        pfid_to_mod: dict[str, tuple[str, dict[str, Any]]],
    ) -> list[tuple[str, str, str, str, str, str, str, str, str, str, str | None]]:
        """
        Build raw row data as plain tuples (no Qt objects).

        Safe to call from any thread. Returns a list of tuples:
        (name, authors, packageid, pfid, supported_versions, downloaded_time,
         updated_time, source, path, workshop_url, uuid)
        """
        rows: list = []
        for pfid, source, timeupdated in acf_entries:
            try:
