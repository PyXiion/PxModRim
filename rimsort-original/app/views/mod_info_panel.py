...
from app.utils.aux_db_utils import auxdb_get_aux_db_entry, auxdb_get_mod_tags
...
class ModInfoPanel:
    ...
    def update_user_mod_notes(self) -> None:
        ...
        uuid = self.current_mod_item.data(Qt.ItemDataRole.UserRole + 1)
        if not uuid:
        ...
        logger.debug(f"Finished updating notes for path: {mod_path}")
    ...
    def show_user_mod_notes(self, item: CustomListWidgetItem) -> None:
        ...
        mod_notes = ""
        path = uuid or "unknown"
        if uuid:
            aux_entry = auxdb_get_aux_db_entry(
                self.settings, uuid, self.metadata_controller.metadata_db_controller
            )
            if aux_entry is not None:
                mod_notes = aux_entry.user_notes or ""
        self.notes.blockSignals(True)
        self.notes.setText(mod_notes)
        self.notes.blockSignals(False)
        logger.debug(f"Finished setting notes for path: {path}")
