...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.main_content_panel.acf_log_reader = self.acf_log_reader
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        ...
        """
        logger.warning("CLOSE: closeEvent starting")

        # Abort any in-progress metadata scanning so background threads stop
        self.metadata_controller.is_abort_requested = True

        # Cancel any in-progress loading task so the startup sequence
        # can unwind and the process can exit
        self.main_content_panel.abort_loading()
        logger.warning("CLOSE: abort_loading done")

        # ACF population runs inline in _do_refresh — no separate task to cancel.
        # If a refresh is in progress, abort_loading() above already cancelled it.

        # Stop filesystem watchdog if running
        self.shutdown_watchdog()
        logger.warning("CLOSE: watchdog done")

        # Close all child windows
        self.main_content_panel.close_child_windows()
        logger.warning("CLOSE: child windows done")

        event.accept()
        logger.warning("CLOSE: event accepted")
        import threading

        logger.warning(f"CLOSE: active threads={threading.enumerate()}")
        import asyncio

        all_tasks = asyncio.all_tasks()
        if all_tasks:
            logger.warning(f"CLOSE: pending asyncio tasks={len(all_tasks)}")
            for t in all_tasks:
                logger.warning(
                    f"CLOSE:   task={t} done={t.done()} cancelled={t.cancelled()}"
                )
        else:
            logger.warning("CLOSE: no pending asyncio tasks")

    def showEvent(self, event: QShowEvent) -> None:
        # Call the original showEvent handler
        super().showEvent(event)

    async def initialize_content(self, is_initial: bool = True) -> None:
        with get_loading_manager().loading_state(self.tr("Initializing...")):
            ...
            await self.main_content_panel._do_refresh(is_initial=is_initial)

            # If the window was closed during scanning, skip remaining initialization
            if not self.isVisible():
                return

            with get_loading_manager().loading_state(self.tr("Checking SteamCMD...")):
            ...
            if is_initial:
                await self.main_content_controller._update_databases_on_startup_if_enabled_silent()

            # CHECK USER PREFERENCE FOR WATCHDOG
            if self.settings.watchdog_toggle:
                # Setup watchdog
                await self.initialize_watchdog()

            with get_loading_manager().loading_state(
                self.tr("Checking Steam integration...")
            ):
                # sleeping to avoid lags
                await asyncio.sleep(0)
    ...
    @asyncSlot(str)
    async def __switch_to_instance(self, instance: str) -> None:
        ...
        # Initialize content
        await self.initialize_content(is_initial=False)
    ...
    async def initialize_watchdog(self) -> None:
        with get_loading_manager().loading_state(
            self.tr("Initializing watchdog FS Observer...")
        ):
            ...
            self.watchdog_event_handler.acf_changed.connect(
                partial(
                    refresh_acf_metadata, self.main_content_panel.metadata_controller
                )
            )
            ...
            await loop.run_in_executor(None, self.watchdog_event_handler.start)
