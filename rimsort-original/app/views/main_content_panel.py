...
from app.models.animations import get_loading_manager
...
class MainContent(QObject):
    ...
    def _connect_signals(self) -> None:
        ...
        EventBus().do_refresh_mods_lists.connect(self._do_refresh_slot)
        ...
        self.mods_panel.active_mods_list.refresh_signal.connect(self._do_refresh_slot)
        self.mods_panel.inactive_mods_list.refresh_signal.connect(self._do_refresh_slot)
    ...
    def _init_state(self) -> None:
        ...
        self._loading_task: asyncio.Future[Any] | None = None
    ...
    @asyncSlot()
    async def _on_metadata_refreshed(self) -> None:
        ...
        if self._refresh_in_progress:
            return
        await self.__repopulate_lists()
        self.mods_panel.refresh_all_tag_filter_selectors()

        if self.acf_log_reader is not None:
            await self.acf_log_reader._populate_from_metadata_async()

    def abort_loading(self) -> None:
        """Cancel any in-progress loading task.

        Called from MainWindow.closeEvent to abort startup scanning so
    ...
    def _insert_data_into_lists(
        ...
        active_list = self.mods_panel.active_mods_list
        inactive_list = self.mods_panel.inactive_mods_list

        live_dividers = active_list.get_dividers_data()
        if live_dividers:
            self.settings.active_mods_dividers = live_dividers
        saved_dividers = self.settings.active_mods_dividers

        total = len(active_mods_uuids) + len(inactive_mods_uuids)
        done = 0

        active_list.recreate_mod_list(
            list_type="active",
            uuids=active_mods_uuids,
        )
        done += len(active_mods_uuids)
        if progress_state is not None:
            progress_state.set_progress(done, total)
        if saved_dividers:
            active_list.restore_dividers(saved_dividers)
        sort_key = ModsPanelSortKey[self.mods_panel.inactive_mods_sort_key]
        descending = self.mods_panel.inactive_sort_descending
        inactive_list.recreate_mod_list_and_sort(
            list_type="inactive",
            uuids=inactive_mods_uuids,
            key=sort_key,
            descending=descending,
        )
        done += len(inactive_mods_uuids)
        if progress_state is not None:
            progress_state.set_progress(done, total)
    ...
    def __mod_list_slot(self, uuid: str, item: CustomListWidgetItem) -> None:
        ...
        """
        self.mod_info_panel.display_mod_info(uuid)
        self.mod_info_panel.show_user_mod_notes(item)

    async def __repopulate_lists(self, is_initial: bool = False) -> None:
        """
        ...
        restore variables.
        """
        with self._loading_manager.loading_state(
            self.tr("Refreshing mod lists...")
        ) as state:
            ...
            # Read ModsConfig.xml off the main thread (I/O-bound)
            (
                active_mods_uuids,
                inactive_mods_uuids,
                self.duplicate_mods,
                ...
                mod_list=mods_config_path,
            )
            _t1 = time.perf_counter()
            logger.warning(f"TIMING repopulate: get_mods_from_list in {_t1 - _t0:.3f}s")
            ...
            self._insert_data_into_lists(active_mods_uuids, inactive_mods_uuids, state)
    ...
    async def _do_check_for_update(self) -> None:
        ...
        await update_manager.do_check_for_update()
    ...
    async def _run_loading(
        self,
        target: Callable[..., Any],
        gif_path: str,
        text: str | None = None,
    ) -> Any:
        """Run target in a thread pool with a modal loading overlay.

        :param target: callable to execute in background thread
        :param gif_path: path to animated GIF to display
        :param text: optional text label shown below the animation
        :returns: return value of target, or None if cancelled
        """
        with self._loading_manager.loading_state(text or "", gif_path=gif_path):
            loop = asyncio.get_running_loop()
            future = loop.run_in_executor(None, target)
            self._loading_task = future
            try:
                result = await future
                return result
            except asyncio.CancelledError:
                logger.debug("Loading task was cancelled")
                return None
            finally:
    ...
    @property
    def _loading_manager(self):
        from app.models.animations import get_loading_manager

        return get_loading_manager()

    # ACTIONS PANEL

    @asyncSlot()
    async def _do_refresh_slot(self, is_initial: bool = False) -> None:
        await self._do_refresh()

    async def _do_refresh(self, is_initial: bool = False) -> None:
        """
        ...
        _t1 = time.perf_counter()
        logger.warning(f"TIMING refresh: setup in {_t1 - _t0:.3f}s")

        with self._loading_manager.loading_state("Refreshing..."):
            # Check if paths are set
            if self.check_if_essential_paths_are_set(prompt=is_initial):
                with self._loading_manager.loading_state(
                    self.tr("Scanning mod sources...")
                ) as scan_state:
                    ...
                    try:
                        ...
                        future = loop.run_in_executor(
                            None,
                            lambda: self.metadata_controller.refresh_metadata(
                                progress_callback=lambda c, t: scan_state.set_progress(
                                    c, t
                                )
                            ),
                        )
                ...
                logger.warning(f"TIMING refresh: metadata scan in {_t2 - _t1:.3f}s")
                ...
                # Insert mod data into lists

                await self.__repopulate_lists(is_initial=is_initial)
                _t3 = time.perf_counter()
                logger.warning(f"TIMING refresh: repopulate_lists in {_t3 - _t2:.3f}s")
                self.mods_panel.refresh_all_tag_filter_selectors()
                _t5 = time.perf_counter()
                logger.warning(f"TIMING refresh: tag filters in {_t5 - _t3:.3f}s")

                # check if we have duplicate mods, prompt user
                with self._loading_manager.loading_state("Checking duplicate mods..."):
                    await asyncio.sleep(0)
                    _tp0 = time.perf_counter()
                    self.__duplicate_mods_prompt()
                    logger.warning(
                        f"TIMING refresh: duplicate_mods_prompt in {time.perf_counter() - _tp0:.3f}s"
                    )

                # check if we have missing mods, prompt user
                with self._loading_manager.loading_state("Checking missing mods..."):
                    await asyncio.sleep(0)
                    _tp1 = time.perf_counter()
                    self.__missing_mods_prompt()
                    logger.warning(
                        f"TIMING refresh: missing_mods_prompt in {time.perf_counter() - _tp1:.3f}s"
                    )

                # Check if we have mods with missing properties (Package ID and/or Publish Field ID)
                with self._loading_manager.loading_state(
                    "Checking mods with missing properties..."
                ):
                    await asyncio.sleep(0)
                    _tp2 = time.perf_counter()
                    self.__check_and_warn_missing_mod_properties()
                    logger.warning(
                        f"TIMING refresh: missing_props_prompt in {time.perf_counter() - _tp2:.3f}s"
                    )

                # Check Workshop mods for updates if configured
                if self.settings.steam_mods_update_check:
                    with self._loading_manager.loading_state(
                        "Checking Workshop mods for updates..."
                    ):
                        await asyncio.sleep(0)
                        logger.info("Checking Workshop mods for updates...")
                        await self._do_check_for_workshop_updates()
                        _t6 = time.perf_counter()
                        logger.warning(
                            f"TIMING refresh: workshop check in {_t6 - _t5:.3f}s"
                        )
                else:
                    logger.info(
                ...
                logger.warning(
                    "Essential paths have not been set. Resetting mod lists."
                )
                if (
        ...
        # Must emit outside the loading_state context manager: pop() calls
        # processEvents() which fires any pending zero-delay Qt timers.
        # If we emitted inside the with block, those timers could invoke
        # _enter_task() while _do_refresh is still the current task.
        EventBus().refresh_finished.emit()
        _t7 = time.perf_counter()
        logger.warning(
            f"TIMING refresh: total after metadata in {_t7 - _t0} - {_t1 - _t0:.3f}s setup"
        )

        # # Inline ACF population — no separate task/timer, avoids the
        # # zero-delay Qt timer race (qasync call_soon → startTimer(0))
        # # that crashes Python 3.12's _enter_task re-entrancy guard.
        # if self.acf_log_reader is not None:
        #     await self.acf_log_reader._populate_from_metadata_async()
    ...
    @asyncSlot(Path)
    async def _upload_file(self, path: Path | None) -> None:
        ...
        success, ret = await self._run_loading(
    ...
    @asyncSlot()
    async def _do_optimize_textures(
        self, block_until_complete: bool = False
    ) -> tuple[bool, int] | None:
        logger.info("Optimizing textures with todds...")
        with get_loading_manager().loading_state("Optimizing textures..."):
            ...
            if block_until_complete:
                loop = asyncio.get_running_loop()
                exit_code_future = loop.create_future()
                todds_runner.process.finished.connect(
                    lambda: exit_code_future.set_result(todds_runner.process.exitCode())
                )
                exit_code = await exit_code_future
                success = exit_code == 0
                if success:
    ...
    @asyncSlot()
    async def _do_check_for_workshop_updates(self) -> None:
        if not check_internet_connection():
            return

        with self._loading_manager.loading_state(
            self.tr("Checking Workshop mods for updates..."),
            "steam_api.gif"
        ) as state:
            loop = asyncio.get_running_loop()
            result: WorkshopUpdateResult = await loop.run_in_executor(
                None,
                partial(
                    query_workshop_update_data,
    ...
    def _do_steamworks_api_call_animated(
        ...
        asyncio.create_task(
            self._run_loading(
                gif_path=str(
                    AppInfo().theme_data_folder / "default-icons" / "steam.gif"
                ),
    ...
    @asyncSlot()
    async def _do_run_game(self) -> None:
        """
        ...
        if self.settings.auto_run_todds_before_launch:
            result = await self._do_optimize_textures(block_until_complete=True)
