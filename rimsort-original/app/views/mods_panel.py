...
class ModListItemInner(QWidget):
    ...
    def repolish(self, item: CustomListWidgetItem) -> None:
        ...
        # Fetch current state from _mod_state — errors/warnings/is_new/etc.
        # live here after recalculate_internal_errors_warnings, NOT on UserRole.
        list_widget = cast(ModListWidget | None, item.listWidget())
        uuid = list_widget.get_uuid(item) if list_widget else None
        state = list_widget._mod_state.get(uuid, {}) if list_widget and uuid else {}

        error_tooltip = state.get("errors", "")
        warning_tooltip = state.get("warnings", "")
        is_new = bool(state.get("is_new", False))
        in_save = bool(state.get("in_save", False))
        is_recently_updated = bool(state.get("is_recently_updated", False))
        updated_timestamp = state.get("updated_timestamp")
        startup_impact_s = state.get("startup_impact_s")
        startup_impact_tooltip = state.get("startup_impact_tooltip", "")
        item_filtered = state.get("filtered", False)
        mod_color = state.get("mod_color", item_data["mod_color"])
        ...
        elif not self.settings.mod_list_updated_indicator or not is_recently_updated:
            self.updated_icon_label.setHidden(True)
            self.updated_icon_label.setToolTip("")
        else:
        ...
        if self.settings.mod_list_startup_impact and startup_impact_s is not None:
            ...
            self.startup_impact_label.setToolTip(startup_impact_tooltip)
...
class ModListWidget(QListWidget):
    """
    Subclass for QListWidget. Stores UUIDs per item (UserRole+1).
    Painting is done via ModItemDelegate — no per-item widgets.
    """
    ...
    def __init__(
        ...
        # Use delegate for painting instead of setItemWidget
        from app.views.mod_list_delegate import ModItemDelegate

        self._delegate = ModItemDelegate(
            metadata_controller, settings, list_widget=self
        )
        ...
        self.model().rowsInserted.connect(
            self._on_rows_inserted, Qt.ConnectionType.QueuedConnection
        )
        self.model().rowsAboutToBeRemoved.connect(
            self._on_rows_about_to_be_removed, Qt.ConnectionType.QueuedConnection
        )
    ...
    def on_selection_changed(
        ...
        pass  # selection highlighting is handled by the delegate
    ...
    def dropEvent(self, event: QDropEvent) -> None:
        source = event.source()
        if source is not self and isinstance(source, ModListWidget):
            # Cross-list drag: move items manually to preserve QListWidgetItem
            # data (UserRole+1 UUID). Qt's default dropEvent creates *new*
            # items that lose the UUID, breaking _on_rows_inserted path tracking.
            items = sorted(
                source.selectedItems(),
                key=lambda item: source.row(item),
            )
            if not items:
                event.ignore()
                return
            ...
            target_idx = self.indexAt(target_pos)
            target_row = target_idx.row() if target_idx.isValid() else self.count()
    ...
    def _get_selected_metadata(self) -> list[dict[str, Any]]:
        ...
        for source_item in selected_items:
            uuid = source_item.data(Qt.ItemDataRole.UserRole + 1)
    ...
    def eventFilter(self, object: QObject, event: QEvent) -> bool:
        ...
        if event.type() == QEvent.Type.ContextMenu and object is self:
            ...
            if item_uuid and is_divider_uuid(item_uuid):
                return self._show_divider_context_menu(item, item_uuid, pos_local)
    ...
    def append_new_item(self, uuid: str) -> None:
        ...
        # Data is set before addItem so _on_rows_inserted sees it immediately
        ...
        item.setData(Qt.ItemDataRole.UserRole + 1, uuid)
        self.addItem(item)
    ...
    def get_all_mod_list_items(self) -> list[CustomListWidgetItem]:
        ...
        for index in range(self.count()):
            ...
            uuid = item.data(Qt.ItemDataRole.UserRole + 1)
            if uuid and is_divider_uuid(uuid):
                continue
    ...
    def get_all_toggled_mod_list_items(self) -> list[QListWidgetItem]:
        items: list[QListWidgetItem] = []
        for i in range(self.count()):
            item = self.item(i)
            uuid = self.get_uuid(item)
            if uuid and self._mod_state.get(uuid, {}).get("warning_toggled", False):
                items.append(item)
        return items

    def get_all_loaded_and_toggled_mod_list_items(self) -> list[QListWidgetItem]:
        return self.get_all_toggled_mod_list_items()

    @staticmethod
    def get_uuid(item: QListWidgetItem | None) -> str | None:
        if item is None:
            return None
        return item.data(Qt.ItemDataRole.UserRole + 1)

    @staticmethod
    def set_uuid(item: QListWidgetItem, uuid: str) -> None:
        item.setData(Qt.ItemDataRole.UserRole + 1, uuid)

    @staticmethod
    def get_divider_name(item: QListWidgetItem) -> str:
        return item.data(Qt.ItemDataRole.DisplayRole) or ""

    @staticmethod
    def set_divider_name(item: QListWidgetItem, name: str) -> None:
        item.setData(Qt.ItemDataRole.DisplayRole, name)

    def check_item_visible(self, item: CustomListWidgetItem) -> bool:
        return False  # no-op: delegate handles visibility

    def create_widget_for_item(self, item: CustomListWidgetItem) -> None:
        pass  # no-op: delegate handles painting
    ...
    def handle_item_data_changed(self, item: CustomListWidgetItem) -> None:
        pass  # no-op: delegate handles repolish

    def handle_other_list_row_added(self, uuid: str) -> None:
        pass  # no-op: delegate doesn't need per-item tracking
    ...
    def _on_rows_inserted(self, parent: QModelIndex, first: int, last: int) -> None:
        ...
        for idx in range(first, last + 1):
            ...
            if item is None:
                continue
            uuid = self.get_uuid(item)
            if uuid and uuid not in self.paths:
    ...
    def _on_rows_about_to_be_removed(
        ...
        for idx in range(last, first - 1, -1):
            ...
            if item is None:
                continue
            uuid = self.get_uuid(item)
            if uuid and uuid in self.paths and idx < len(self.paths):
                self.paths.pop(idx)
        if len(self.paths) == self.count():
            logger.debug(
    ...
    def add_divider(self, index: int, name: str) -> None:
        ...
        item.setData(Qt.ItemDataRole.UserRole + 1, uuid)
        try:
            self.model().rowsInserted.disconnect(self._on_rows_inserted)
        except TypeError:
            pass
        ...
        self.model().rowsInserted.connect(
            self._on_rows_inserted, Qt.ConnectionType.DirectConnection
        )
        self.apply_collapse_states()
        self.list_update_signal.emit(str(self.count()))
    ...
    def restore_dividers(
        self,
        dividers: list[dict[str, Any]],
        defer_visible_widgets: bool = False,
    ) -> None:
        ...
        for div in sorted_dividers:
            ...
            data = DividerData(uuid=uuid, name=name, collapsed=collapsed)
    ...
    def mod_changed_to(
        ...
        if current is not None:
            uuid = current.data(Qt.ItemDataRole.UserRole + 1)
            if uuid is None:
                return
            if is_divider_uuid(uuid):
                return
            self.mod_info_signal.emit(uuid, current)
    ...
    def mod_clicked(self, current: CustomListWidgetItem) -> None:
        if current is not None:
            ...
            mod = self.metadata_controller.get_mod(uuid)
            logger.debug(
                f"USER ACTION: mod was clicked: [{uuid}] {mod.name if mod else 'unknown'}"
            )
    ...
    def mod_double_clicked(self, item: CustomListWidgetItem) -> None:
        ...
        if uuid is None:
            return
        if is_divider_uuid(uuid):
            self.toggle_divider_collapse(uuid)
            return
    ...
    def _check_use_this_instead(self, uuid: str) -> bool:
        """Check if use_this_instead is applicable."""
        return bool(self.metadata_controller.has_alternative_mod(uuid))
    ...
    def recalculate_internal_errors_warnings(self) -> tuple[str, str, int, int]:
        ...
        for uuid, mod_errors in package_id_to_errors.items():
            ...
            state = self._mod_state.setdefault(uuid, {})
            ...
            if save_compare_enabled:
                try:
                    ...
                    if self.list_type == "Active":
                        state["is_new"] = not is_in_save
                        state["in_save"] = False
                    else:
                        state["is_new"] = False
                        state["in_save"] = is_in_save
                except Exception:
                    state["is_new"] = False
                    state["in_save"] = False
            else:
            ...
            if updated_enabled:
                ...
                if (
                    ...
                    and mod_obj.mod_type in (ModType.STEAM_WORKSHOP, ModType.STEAM_CMD)
                ):
                    aux_entry = auxdb_get_aux_db_entry(self.settings, uuid)
                    state["updated_timestamp"] = resolve_workshop_updated_timestamp(
                        aux_entry
                    )
                else:
                    state["updated_timestamp"] = None
                ts = state.get("updated_timestamp")
                state["is_recently_updated"] = bool(
                    ts is not None and ts >= updated_cutoff
                )
            else:
                state["updated_timestamp"] = None
                state["is_recently_updated"] = False

            alt_info = self.metadata_controller.has_alternative_mod(uuid)
            state["alternative"] = (
                f"{alt_info.name} ({alt_info.pfid}) by {alt_info.author}"
                if alt_info
                else ""
            )
            ...
            if impact_entry is not None and startup_impact_report is not None:
                state["startup_impact_s"] = impact_entry.total_impact_s
                state["startup_impact_tooltip"] = self._build_startup_impact_tooltip(
                    impact_entry, startup_impact_report
                )
            else:
                state["startup_impact_s"] = None
                state["startup_impact_tooltip"] = ""
            ...
            if pkg_id_str not in self.ignore_warning_list and not state.get(
                "warning_toggled", False
            ):
                state["mismatch"] = mod_errors["version_mismatch"]
            else:
                if not state.get("warning_toggled", False):
            ...
            if (
                self._check_use_this_instead(uuid)
                and pkg_id_str not in self.ignore_warning_list
            ):
                mod_errors["use_this_instead"] = True
                tool_tip_text += self.tr(
                    "\nAn alternative updated mod is recommended:\n{alternative}"
                ).format(alternative=state.get("alternative", ""))
            ...
            state["errors_warnings"] = tool_tip_text.strip()
            state["warnings"] = tool_tip_text[len(state.get("errors", "")) :].strip()
            state["errors"] = state.get("errors", "").strip()
        _t1 = time.perf_counter()
        logger.warning(
            f"TIMING: recalculate_internal_errors_warnings ({self.list_type}) in {_t1 - _t0:.3f}s"
        )
    ...
    def recreate_mod_list(
        ...
        try:
            ...
            pass
        try:
            self.model().rowsAboutToBeRemoved.disconnect(
        ...
        with aux_metadata_controller.Session() as aux_metadata_session:
            for uuid_key in uuids:
                if is_divider_uuid(uuid_key):
                    continue
                mod = self.metadata_controller.get_mod(uuid_key)
                mod_path = str(mod.mod_path) if mod and mod.mod_path else uuid_key
                aux_metadata_controller.get_or_create(aux_metadata_session, mod_path)
                aux_metadata_controller.update(
                    aux_metadata_session, mod_path, outdated=False
                )
                ...
                data.__dict__["show_tags"] = self.show_tags
                list_item = CustomListWidgetItem()
                list_item.setData(Qt.ItemDataRole.UserRole, data, avoid_emit=True)
                list_item.setData(Qt.ItemDataRole.UserRole + 1, uuid_key)
        ...
        self.model().rowsAboutToBeRemoved.connect(
            self._on_rows_about_to_be_removed, Qt.ConnectionType.DirectConnection
        )

        self.list_update_signal.emit(str(self.count()))

    def toggle_warning(self, packageid: str | None, uuid: str) -> None:
        logger.debug(f"Toggled warning icon for: {packageid}")
        state = self._mod_state.setdefault(uuid, {"warning_toggled": False})
        if packageid is None:
            return
        if packageid not in self.ignore_warning_list:
            self.ignore_warning_list.append(packageid)
            state["warning_toggled"] = True
        else:
            self.ignore_warning_list.remove(packageid)
            state["warning_toggled"] = False
        aux_metadata_controller = AuxMetadataController.get_or_create_cached_instance(
            self.settings.aux_db_path
        )
        with aux_metadata_controller.Session() as aux_metadata_session:
    ...
    def refresh_mod_tags_for_uuid(self, uuid: str) -> None:
        ...
        state = self._mod_state.setdefault(uuid, {})
        state["mod_tags"] = tags
        state["show_tags"] = self.show_tags

        current_mod_index = self.paths.index(uuid)
        item = self.item(current_mod_index)
        item_data = item.data(Qt.ItemDataRole.UserRole)
        if item_data is not None:
            item_data["mod_tags"] = tags
        ...
        if isinstance(widget, ModListItemInner):
            widget.set_tags_visible(
                item_data.__dict__["show_tags"] if item_data else self.show_tags,
                tags,
            )
    ...
    def set_tags_visible(self, visible: bool) -> None:
        ...
        for index in range(self.count()):
            ...
            if item_data is None:
                continue
            if getattr(item_data, "is_divider", False) or not uuid:
                continue
...
class ModsPanel(QWidget):
    ...
    def _on_folder_size_finished(self, sizes: dict[str, int]) -> None:
        ...
        try:
            ...
            try:
                lw.model().rowsInserted.disconnect(lw._on_rows_inserted)
            except TypeError:
                pass  # Signal not connected
            try:
                lw.model().rowsAboutToBeRemoved.disconnect(
                    lw._on_rows_about_to_be_removed
                )
            except TypeError:
                pass  # Signal not connected
            ...
            # Single DB session for aux metadata updates
            with aux_metadata_controller.Session() as aux_metadata_session:
            ...
            lw.model().rowsInserted.connect(
                lw._on_rows_inserted, Qt.ConnectionType.DirectConnection
            )
            lw.model().rowsAboutToBeRemoved.connect(
                lw._on_rows_about_to_be_removed, Qt.ConnectionType.DirectConnection
            )
    ...
    def recalculate_list_errors_warnings(self, list_type: str) -> None:
            ...
            if self.settings.show_save_comparison_indicators:
                try:
                    for item in self.active_mods_list.get_all_mod_list_items():
                        uuid = item.data(Qt.ItemDataRole.UserRole + 1)
                        if uuid:
                            state = self.active_mods_list._mod_state.get(uuid, {})
            ...
            if self.settings.mod_list_updated_indicator:
                try:
                    for item in self.active_mods_list.get_all_mod_list_items():
                        ...
                        if uuid:
                            state = self.active_mods_list._mod_state.get(uuid, {})
                            if state.get("is_recently_updated", False):
                                updated_count += 1
                except Exception:
                    pass
        ...
        logger.warning(
            f"TIMING: recalculate_list_errors_warnings ({list_type}) in {time.perf_counter() - _t0:.3f}s"
        )
    ...
    def signal_search_and_filters(
        ...
        for idx, uuid in enumerate(uuids):
            ...
            item_data = mod_list._mod_state.setdefault(uuid, {})
            ...
            if self.settings.hide_invalid_mods_when_filtering:
                invalid = item_data.get("invalid", False)
            ...
            item_filtered = item_data.get("filtered", False)
            ...
            # Update item data (item_data is _mod_state[uuid]; no write-back to UserRole)
            item_data["path"] = uuid
        ...
        if list_type == "Active":
            ...
            self.inactive_mods_list.check_widgets_visible()
        logger.warning(
            f"TIMING: signal_search_and_filters ({list_type}) in {time.perf_counter() - _t0:.3f}s"
        )
        ...
        for uuid in mods_to_add:
            if uuid not in self.active_mods_list.paths:
                ...
                if uuid in self.inactive_mods_list.paths:
                    ...
                    # self.active_mods_list.paths is updated via _on_rows_inserted signal

                    # Ensure _mod_state is updated (list_type)
                    state = self.active_mods_list._mod_state.setdefault(uuid, {})
                    state["list_type"] = "Active"
