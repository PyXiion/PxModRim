...
async def _await_qthread_signal(signal: Any) -> tuple[Any, ...]:
    """Await a QThread worker's completion signal, returning emitted args."""
...
class MainContentController(QObject):
    ...
    async def _update_databases_on_startup_if_enabled_silent(self) -> None:
        """
        ...
        with get_loading_manager().loading_state(self.tr("Updating databases...")) as state:
            await asyncio.sleep(0)
            settings = self.settings
            http_tasks: list[DatabaseDownloadTask] = []
            db_coros: list[asyncio.Task[None]] = []
                ...
                if source == "Configured URL" and url:
                    ...
                    db_coros.append(
                        asyncio.ensure_future(
                            self._do_auto_database_update_async(
            ...
            if http_tasks:
                db_coros.append(
                    asyncio.ensure_future(
                        self._start_http_download_async(
                            http_tasks, self._notify_http_result_silent
                        )
                    )
                )
    ...
    async def _do_auto_database_update_async(
        self, base_path: str, repo_url: str
    ) -> None:
        """Async variant: await the clone/pull worker instead of fire-and-forget."""
        logger.info(f"Starting async database update: {repo_url}")
        parsed = git_utils.parse_git_url(repo_url)
        if parsed is None:
            logger.error(f"Invalid git URL for database update: {repo_url}")
            return
        ...
        if full_repo_path.exists():
            logger.info(f"Updating existing database repository: {full_repo_path}")
            filtered_paths = self._filter_non_github_repos([full_repo_path])
            if not filtered_paths:
                return
            config = GitOperationConfig(notify_errors=False)
            worker = GitBatchUpdateWorker(filtered_paths, config=config)
            self.thread_pool.start(worker)
            await _await_qthread_signal(worker.signals.finished)
        else:
            logger.info(f"Cloning new database repository to: {full_repo_path}")
            config = GitOperationConfig(notify_errors=False)
            ...
            worker.start()
            await _await_qthread_signal(worker.finished)
    ...
    async def _start_http_download_async(
        self,
        tasks: list[DatabaseDownloadTask],
        notify: Callable[[list[str], list[str], list[str]], None],
    ) -> None:
        """Async variant: await the download worker instead of fire-and-forget."""
        worker = HttpDownloadWorker(tasks)
        worker.progress.connect(lambda msg: logger.info(f"HTTP DB download: {msg}"))
        worker.download_finished.connect(
            lambda results: self._on_http_download_finished(results, notify)
        )
        logger.info(f"Starting async HTTP download for {len(tasks)} database(s)")
        worker.start()
        await _await_qthread_signal(worker.finished)
