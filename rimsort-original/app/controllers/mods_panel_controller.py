...
class ModsPanelController(QObject):
    ...
    def _on_menu_bar_reset_warnings_triggered(self) -> None:
        ...
        for mod_list in [
            self.mods_panel.active_mods_list,
            self.mods_panel.inactive_mods_list,
        ]:
            for item in mod_list.get_all_toggled_mod_list_items():
                uuid = mod_list.get_uuid(item)
                if not uuid:
                    continue
                mod_obj = metadata_controller.get_mod(uuid)
                package_id = (
                    str(mod_obj.package_id)
                    if mod_obj and isinstance(mod_obj, AboutXmlMod)
                    else None
                )
                mod_list.toggle_warning(package_id, uuid)
    ...
    def __change_visibility_helper(self, label_active: bool, type: str) -> None:
        mod_list = self.mods_panel.active_mods_list
        active_mods = mod_list.get_all_mod_list_items()
        for mod in active_mods:
            uuid = mod_list.get_uuid(mod)
            if not uuid:
                continue
            state = mod_list._mod_state.get(uuid, {})
            if type == "new_text":
                apply_filter = not bool(state.get("is_new", False))
            elif type == "updated_text":
                apply_filter = not bool(state.get("is_recently_updated", False))
            else:
                apply_filter = state.get(type, "") == ""

            # If a mod is already hidden becasue of filters, dont unhide it
            if apply_filter:
                if label_active:
                    mod.setHidden(True)
                elif not state.get("hidden_by_filter", False):
                    mod.setHidden(False)
