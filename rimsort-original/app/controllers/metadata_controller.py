...
from app.models.metadata.metadata_factory import parse_mod_batch
...
class MetadataController(QObject):
    ...
    def refresh_metadata(
        ...
        with self.metadata_db_controller.Session() as session:
            mods = list(self.metadata_mediator.mods_metadata.items())
            total = len(mods)
            for i, (path, mod_data) in enumerate(mods, 1):
                try:
    ...
    def _parse_single_mod(self, mod_path: str) -> bool:
        ...
        prefer_versioned = self.settings.prefer_versioned_about_tags
        results = parse_mod_batch(
            [mod_path],
            self.metadata_mediator.game_version,
            str(self.metadata_mediator.local_mods_path),
            str(self.metadata_mediator.game_path),
            str(self.metadata_mediator.workshop_mods_path)
            if self.metadata_mediator.workshop_mods_path
            else None,
        ...
        self.metadata_mediator._mods_metadata.update(results)
