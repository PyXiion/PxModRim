...
class AppController(QObject):
    ...
    async def _async_run(self) -> int:
        ...
        await self.main_window.initialize_content(is_initial=True)
        if not self.main_window.isVisible():
            return 0
        app_close_event = asyncio.Event()
        self.app.aboutToQuit.connect(app_close_event.set)
        await app_close_event.wait()
        logger.warning("CLOSE: app_close_event set, _async_run returning")
        return 0

    def run(self) -> int:
        """Runs the main application loop after initializing the main window."""
        return asyncio.run(self._async_run(), loop_factory=QEventLoop)
