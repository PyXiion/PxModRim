...
def parse_mod_batch(
    ...
    """Parse a batch of mod paths into ListedMod objects in a subprocess.

    This is a top-level function specifically designed to be pickle-able
    for use with ``ProcessPoolExecutor``. No Qt objects, no threading
    primitives, no GIL — pure CPU work.

    :param mod_paths: List of absolute filesystem paths to mod directories
    :param target_version: The RimWorld version string for version-specific rules
    :param local_path: Path to the local mods directory
    :param rimworld_path: Path to the RimWorld game directory
    :param workshop_path: Path to the workshop mods directory (may be None)
    :param user_rules: User-defined external rules (may be None)
    :param community_rules: Community-defined external rules (may be None)
    :param steam_db: Steam database schema (may be None)
    :param prefer_versioned: When True, ByVersion keys override base values
    :param case_insensitive_about_xml: When True, use case-insensitive About.xml lookup
    :returns: Dict mapping UUID (filesystem path) to ``ListedMod``
    """
    results: dict[str, ListedMod] = {}
    local = Path(local_path)
    rimworld = Path(rimworld_path)
    workshop = Path(workshop_path) if workshop_path else None
